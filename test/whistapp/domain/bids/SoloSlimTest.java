...
class SoloSlimTest {
    ...
    void testCalculatePoints_legal() {
        assertEquals(-90, soloSlim.calculatePoints(tricksWon).get(p1));
        assertEquals(30, soloSlim.calculatePoints(tricksWon).get(p2));
        assertEquals(30, soloSlim.calculatePoints(tricksWon).get(p3));
        assertEquals(30, soloSlim.calculatePoints(tricksWon).get(p4));

        assertEquals(90, soloSlim.calculatePoints(tricksWon5).get(p1));
        assertEquals(-30, soloSlim.calculatePoints(tricksWon5).get(p2));
        assertEquals(-30, soloSlim.calculatePoints(tricksWon5).get(p3));
        assertEquals(-30, soloSlim.calculatePoints(tricksWon5).get(p4));
    }

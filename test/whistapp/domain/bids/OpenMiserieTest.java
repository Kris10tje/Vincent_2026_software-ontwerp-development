...
class OpenMiserieTest {
    ...
    void testGetOpenMiserieHands() {
        ...
        HashMap<Player, String[]> hands = openMiserie3.getOpenMiserieHands(p1);
        assertEquals(3, hands.size());
        assertEquals(hands.get(p2).length, 13);
        assertEquals(p2.getHandCards()[0], hands.get(p2)[0]);
        ...
        assertEquals(13, hands.get(p3).length);
        assertEquals(p3.getHandCards()[0], hands.get(p3)[0]);
        hands = openMiserie3.getOpenMiserieHands(p3);
        assertEquals(2, hands.size());
        assertFalse(hands.containsKey(p3));
        assertEquals(13, hands.get(p4).length);
        assertEquals(p4.getHandCards()[0], hands.get(p4)[0]);
        hands = openMiserie3.getOpenMiserieHands(p4);
        assertEquals(2, hands.size());
        assertFalse(hands.containsKey(p4));
        assertEquals(13, hands.get(p2).length);
        assertEquals(p2.getHandCards()[0], hands.get(p2)[0]);
    }
